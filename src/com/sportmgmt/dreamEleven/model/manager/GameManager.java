...
public class GameManager {
		...
		else
		{
			...
			if(session != null)
			{
				try
				{
					...
					//SQLQuery query = session.createSQLQuery("select gcp.game_club_player_id from game_club_player gcp, user_player up where gcp.game_club_player_id = up.game_club_player_id and up.user_id =20 and gcp.game_id =1");
					SQLQuery query = session.createSQLQuery(QueryConstrant.SELECT_USER_PLAYER_DETAIL_LIST_ELEVEN);
					query.setParameter("userId", userId);
					query.setParameter("gameId", gameId);
	...
	public static boolean addPlayerOfGametToUserAccountDreamEleven(String userId,String gameClubPlayerId)
	{
	...
	public static boolean removePlayeOfGameFromUserAccountDreamEleven(String userId,String gameClubPlayerId)
	{
	...
	public static int updateUserTotalPoint(String userId,String gameId,int updateWith)
	{
		int userTotalPoint =  0;
	
		setErrorMessage("");
		SessionFactory factory = HibernateSessionFactory.getSessionFacotry();
		logger.info("--------------- getUserTotalPoint ------------> userId:  "+userId+" gameId: "+gameId+", update With:"+updateWith);
		if(factory == null)
		{
			setErrorCode(ErrorConstrant.SESS_FACT_NULL);
			setErrorMessage("Technical Error");
		...
		else
		{
			...
			if(session != null)
			{
				try
				{
					...
					if(results == null || results.size() ==0)
					{
						logger.info(" ------- Enty not found in UserGame");
					}
					else
					{
						...
						{
							userTotalPoint = userGame.getTotalPoint();
							userTotalPoint +=updateWith;
							userGame.setTotalPoint(userTotalPoint);
				...
				{
					logger.error("Exception fetch updateUserTotalPoint: "+ex.getMessage());
					setErrorMessage("Technical Error");
					setErrorCode(ErrorConstrant.TRANSACTION_ERROR);
				}
				finally
